...
#define P2_66       1.355252715606881E-20
...
static int decode_type1047(rtcm_t *rtcm)
{
    ...
    if (i+476<=rtcm->len*8) {
        ...
        eph.iode  =getbitu(rtcm->buff,i, 8);              i+= 8;
        toc       =
        		getbitu(rtcm->buff,i,16)*16.0;         i+=16;
        eph.f2    =getbits(rtcm->buff,i, 8)*P2_55;        i+= 8;
        eph.f1    =getbits(rtcm->buff,i,16)*P2_43;        i+=16;
        eph.f0    =getbits(rtcm->buff,i,22)*P2_31;        i+=22;
        eph.iodc  =getbitu(rtcm->buff,i,10);              i+=10;
    ...
    eph.A=sqrtA*sqrtA;
    if (!strstr(rtcm->opt,"-EPHALL")) {
    	if(eph.week==rtcm->nav.eph[sat-1].week && eph.toe.time==rtcm->nav.eph[sat-1].toe.time) return 0; /*unchanged*/
     //   if (eph.iode==rtcm->nav.eph[sat-1].iode) return 0; /* unchanged */
    }
    rtcm->nav.eph[sat-1]=eph;
    rtcm->ephsat=sat;
...
static int decode_ssr4(rtcm_t *rtcm, int sys)
{
    ...
    for (j=0;j<nsat&&i+199+np<=rtcm->len*8;j++) {
        ...
        if(sys==SYS_CMP){
        	i+=10;
        	iode=getbitu(rtcm->buff,i,24);		i+=24;
        }else{
        	iode=getbitu(rtcm->buff,i, 8); 		i+= 8;
        }
        deph [0]=getbits(rtcm->buff,i,22)*1E-4; i+=22;
...
/*decode type 0063:bds ephemeris*/
static int decode_type0063(rtcm_t *rtcm)
{

    eph_t eph={0};
    double toc,sqrtA,tmp;
    char *msg;
    int i=24+12,prn,sat,week,sys=SYS_CMP,w,sow;
    ...
    if (i+476<=rtcm->len*8) {
        ...
        toc		  =getbitu(rtcm->buff,i,17)<<3;			  i+=17;
        ...
        eph.toes  =fmod((getbitu(rtcm->buff,i,17)*8),604800);  i+=17;
    ...
    sow=time2bdt(gpst2bdt(utc2gpst(timeget())),&w);

    //if(toc==0&&sow>601200) w+=1; //86400*7-3600


    if (eph.toes < 3600 && sow > 79200)
		w += 1;
...
extern int decode_rtcm3(rtcm_t *rtcm)
{
    ...
    trace(2,"decode_rtcm3: len=%3d type=%d\n",rtcm->len,type);
    if (rtcm->outtype) {
    ...
    if(type<1315){
		switch (type) {
			...
			//add from the igmas
			case 1238: ret=decode_ssr4(rtcm,SYS_CMP); break;
			case 1261: ret=decode_ssr4(rtcm,SYS_CMP); break;
			//add from igs message number for galileo ephemeris
			case 1046: ret=decode_type1045(rtcm);     break;
			case   63: ret=decode_type0063(rtcm);     break;
		//	case 1258:ret=decode_ssr4(rtcm,SYS_CMP); break;
		}
		if (ret>=0) {
			type-=1000;
