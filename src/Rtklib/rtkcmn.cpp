...
static char codepris[6][MAXRTKFREQ][16]={  /* code priority table */
...
* return : a'*b
*-----------------------------------------------------------------------------*/
extern double dot_rtk(const double *a, const double *b, int n)
{
    double c=0.0;
...
*-----------------------------------------------------------------------------*/
extern int matinv_rtk(double *A, int n)
{
    double *work;
                 ...
                 int m, double *X)
{
    ...
    matcpy(B,A,n,n);
    if (!(info=matinv_rtk(B,n))) matmul(tr[0]=='N'?"NN":"TN",n,m,n,1.0,B,Y,0.0,X);
    free(B);
    return info;
}
#endif
               ...
               double *Q)
{
    ...
    if (!(info=matinv_rtk(Q,n))) matmul("NN",n,1,n,1.0,Q,Ay,0.0,x); /* x=Q^-1*Ay */
    free(Ay);
    return info;
}
/* kalman filter ---------------------------------------------------------------
                    ...
                    const double *Qb, int n, double *xs, double *Qs)
{
    ...
    matcpy(invQf,Qf,n,n);
    matcpy(invQb,Qb,n,n);
    if (!matinv_rtk(invQf,n)&&!matinv_rtk(invQb,n)) {
        for (i=0;i<n*n;i++) Qs[i]=invQf[i]+invQb[i];
        if (!(info=matinv_rtk(Qs,n))) {
...
{
    double e2=FE_WGS84*(2.0-FE_WGS84),r2=dot_rtk(r,r,2),z,zk,v=RE_WGS84,sinp;
...
*-----------------------------------------------------------------------------*/
extern int readpcv(char *file, pcvs_t *pcvs)
{
    pcv_t *pcv;
    const char *ext;
    int i,stat;
...
extern int readnav(const char *file, nav_t *nav)
{
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        sscanf(p+1,"%d,%d,%d,%d,%d,%d,%d,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,"
...
extern double satwavelen(int sat, int frq, const nav_t *nav)
{
    ...
    if (sys==SYS_GLO) {
        ...
        else if (frq==1 || frq == 4) return CLIGHT/FREQ2_CMP; /* B2 */
        else if (frq==2 || frq == 3) return CLIGHT/FREQ3_CMP; /* B3 */
    }
    else {
    ...
    if(sys == SYS_GLO){
    	...
    	char cprn[16] = {0};
    	int ifq;
    	int ifreqs[] = {1,-4,5,6,1,-4,5,6,-6,-7, // 1-10
    			0,-1,-2,-7,0,-1,4,-3,3,2,     // 11-20
				4,-3,3,2,-5,7                // 21-27
    	};
    	satno2id(sat,cprn);
    	ifq = atoi(cprn + 1);
    	return CLIGHT/(freq_glo[frq]+dfrq_glo[frq]*ifreqs[ifq - 1]);
    }
    return 0.0;
}
/* geometric distance ----------------------------------------------------------
...
extern double satazel(const double *pos, const double *e, double *azel)
{
    ...
    if (pos[2]>-RE_WGS84) {
        ecef2enu(pos,e,enu);
        az=dot_rtk(enu,enu,2)<1E-12?0.0:atan2(enu[0],enu[1]);
        if (az<0.0) az+=2*pi;
        el=asin(enu[2]);
    }
                     ...
                     int opt, double *dant)
{
    ...
    for (i=0;i<NFREQ;i++) {
        ...
        dant[i]=-dot_rtk(off,e,3)+(opt?interpvar(90.0-azel[1]*R2D,pcv->var[i]):0.0);
    }
                       ...
                       double *phw)
{
    ...
    cross3(ek,eyr,ekr);
    for (i=0;i<3;i++) {
        ds[i]=exs[i]-ek[i]*dot_rtk(ek,exs,3)-eks[i];
        dr[i]=exr[i]-ek[i]*dot_rtk(ek,exr,3)+ekr[i];
    }
    cosp=dot_rtk(ds,dr,3)/norm(ds,3)/norm(dr,3);
    if      (cosp<-1.0) cosp=-1.0;
    else if (cosp> 1.0) cosp= 1.0;
    ph=acos(cosp)/2.0/pi;
    cross3(ds,dr,drs);
    if (dot_rtk(ek,drs,3)<0.0) ph=-ph;
